...
- Agent Loop 新增独立分析阶段，采集摘要不再直接冒充重要信号。
- 使用 GitHub Models 阅读正文并输出“核心提炼、关键要点、分析、输出”；GitHub Actions 通过内置 Token 调用，无需新增模型密钥。
- 只有完成结构化分析的内容才进入网站重要信号；未分析内容仍保留在实时情报。
- 已生成的分析随 Feed 保存并跨运行复用，默认每轮最多分析 6 条新信号。
- 知识卡、RSS/JSON 和 React 网站统一采用新的 Insight Contract。
- 测试增至 69 项，严格评测保持 3/3、平均 2.0/2。

## [0.8.0] - 2026-07-29

- 新增可公开访问的 React 情报网站，支持来源筛选、重要信号、实时流和 RSS/JSON 复制。
- 重要信号改为不限三条、分批加载，并可展开查看判断、证据与边界。
- 降低 Nightly、Alpha、Beta、RC 等预发布版本重要度，避免版本噪声占据精选区。
- Feed `_radar` 元数据新增影响分、可信度和短证据，供网站稳定解释和排序。
- 新增可选 Firecrawl v2 通用官方网页适配器；API Key 仅从 GitHub Secret 读取。
- 新增维护者来源管理预览和订阅 Agent 预览，明确当前不在公开前端写配置或保存密钥。
- Python 测试增至 66 项，并将 Web 构建与静态托管测试纳入 CI。
...
- 新增通用 RSS/Atom 采集器；Reddit 改为无需 OAuth 的合并公共 RSS。
- 自动发布器改为动态加载订阅清单，清单变更后立即发布，不再写死来源 ID。
- 新增 `--include-auth`，后台凭证存在时才追加 X；订阅者和公开来源保持零认证。
- Agent 默认步数预算按来源数量自动计算，避免增加来源后触发固定上限。
- 新增订阅修改指南和中英文 README 操作入口。
- 测试增至 53 项；真实无凭证链路读取 55 条、筛选并发布 46 条、0 个错误。
...
- 新增 CI 环境变量授权入口，Cookie 只从 GitHub Secret 读取；缺少配置时只跳过 X 并继续发布公开来源。
